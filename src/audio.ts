...
class AudioHeadless {
  ...
  private static _instance: AudioHeadless;
  ...
  constructor() {
    if (AudioHeadless._instance) {
      console.warn(
        'Instantiation failed: cannot create multiple instance of AudioHeadless returning existing instance',
      );
      // biome-ignore lint/correctness/noConstructorReturn: <explanation>
      return AudioHeadless._instance;
    }
    if (process.env.NODE_ENV !== AUDIO_X_CONSTANTS?.DEVELOPMENT && audioInstance) {
      throw new Error('Cannot create multiple audio instance');
    }

    AudioHeadless._instance = this;
  ...
  /**
   *
   * @param initProps  initial config to initialize AudioHeadless
   ...
   * @param mode mode of operation for AudioHeadless
  ...
  async addMedia(mediaTrack: MediaTrack, mediaFetchFn?: (mediaTrack: MediaTrack) => Promise<void>) {
    ...
    if (mediaType === 'HLS' && !audioInstance.canPlayType('application/vnd.apple.mpegurl')) {
      ...
      if (hlsInstance) {
        ...
        console.warn(
          'The source provided seems to be a HLS stream but, hls playback is not enabled. Please have a look at init method of AudioHeadless',
        );
        await this.reset();
      }
    } else {
